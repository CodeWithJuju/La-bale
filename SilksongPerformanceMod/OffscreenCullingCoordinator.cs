...
internal sealed class OffscreenCullingCoordinator : MonoBehaviour
{
    ...
    private static readonly List<MonoBehaviour> MonoBehaviourBuffer = new(128);

    private static readonly List<Rigidbody2D> Rigidbody2DBuffer = new(4);
    private static readonly List<Collider2D> Collider2DBuffer = new(4);
    ...
    {
        if (ModSettings.EnableAggressiveOffscreenCulling.Value)
        {
            Install();
            return;
        }
    ...
    private void ProcessParticleCandidates(Camera camera, int budget)
    {
        ...
        {
            _particleIndex %= _particleCandidates.Count;
    ...
    private void ProcessRendererCandidates(Camera camera, int budget)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            _rendererIndex %= _rendererCandidates.Count;
    ...
    private void ProcessBehaviourCandidates(Camera camera, int budget)
    {
        ...
        for (int i = 0; i < count; i++)
        {
            _behaviourIndex %= _behaviourCandidates.Count;
    ...
    /// <summary>
    /// CORRIGIDO: ordem de checagens reordenada do mais barato para o mais caro.
    ///
    /// Ordem anterior:
    ///   1. HasRiskyTagOrName (string scan — barato)
    ///   2. GetComponentInParent x2 (traversal hierarquia — caro)
    ///   3. GetComponentInChildren x2 (traversal hierarquia — caro)
    ///   4. ScanMonoBehaviours (GetComponentsInParent + InChildren — caro)
    ///
    /// Ordem nova:
    ///   1. HasRiskyTagOrName (string scan — barato, elimina maioria dos rejeitos)
    ///   2. ScanMonoBehaviours no próprio objeto GetComponents (barato, sem traversal)
    ///   3. GetComponentsInParent/InChildren para físicos (caro, só para candidatos que passaram)
    ///
    /// Na prática a maioria dos rejeitos tem nome/tipo arriscado e cai no passo 1,
    /// nunca chegando nos traversals de hierarquia.
    /// </summary>
    private static bool IsSafeVisualCandidate(GameObject gameObject)
    {
        ...
        if (HasRiskyTagOrName(gameObject))
        {
            return false;
        }

        if (!ScanMonoBehaviours(gameObject, includeInactive: true))
        {
            return false;
        }

        if (HasPhysicsComponents(gameObject))
        {
            return false;
        }
    ...
    /// <summary>
    /// CORRIGIDO: verifica componentes de física usando GetComponentsInParent/InChildren
    /// com lista estática, evitando alocações. Separado de IsSafeVisualCandidate
    /// para ser chamado por último (mais caro).
    /// </summary>
    private static bool HasPhysicsComponents(GameObject gameObject)
    {
        Rigidbody2DBuffer.Clear();
        gameObject.GetComponentsInParent(includeInactive: true, Rigidbody2DBuffer);
        if (Rigidbody2DBuffer.Count > 0)
        {
            Rigidbody2DBuffer.Clear();
            return true;
        }
        ...
        gameObject.GetComponentsInParent(includeInactive: true, Collider2DBuffer);
        if (Collider2DBuffer.Count > 0)
        {
            Collider2DBuffer.Clear();
            return true;
        }

        gameObject.GetComponentsInChildren(includeInactive: true, Rigidbody2DBuffer);
        if (Rigidbody2DBuffer.Count > 0)
        {
            Rigidbody2DBuffer.Clear();
            return true;
        ...
        gameObject.GetComponentsInChildren(includeInactive: true, Collider2DBuffer);
        bool hasCollider = Collider2DBuffer.Count > 0;
        Collider2DBuffer.Clear();
        return hasCollider;
    }

    private static bool ScanMonoBehaviours(GameObject gameObject, bool includeInactive)
    {
        MonoBehaviourBuffer.Clear();
        gameObject.GetComponentsInParent(includeInactive, MonoBehaviourBuffer);
        for (int i = 0; i < MonoBehaviourBuffer.Count; i++)
        {
            if (IsRiskyComponent(MonoBehaviourBuffer[i]))
            {
                MonoBehaviourBuffer.Clear();
                return false;
            }
        ...
        MonoBehaviourBuffer.Clear();
        gameObject.GetComponentsInChildren(includeInactive, MonoBehaviourBuffer);
        for (int i = 0; i < MonoBehaviourBuffer.Count; i++)
        {
            if (IsRiskyComponent(MonoBehaviourBuffer[i]))
            {
