...
internal static class ExternalVisualPatches
{
    ...
    {
        if (!RuntimeContext.ShouldApplyDistanceThrottle() || !ModSettings.EnableNestedFadeMixerThrottle.Value)
        {
            return true;
        }
    ...
    private static bool CullOffscreenAraTrail(AraTrail __instance, Camera cam)
    {
        ...
        if (!ModSettings.EnableAraTrailOffscreenCulling.Value || cam == null)
        {
            return true;
        }
        ...
        // Recalcula os frustum planes apenas uma vez por frame por câmera,
        // em vez de uma vez por trail. Com N trails visíveis, reduz N chamadas para 1.
