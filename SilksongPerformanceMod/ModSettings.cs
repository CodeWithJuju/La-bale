...
{
    public static ConfigEntry<bool> EnableHighFpsCompatibilityMode { get; private set; } = null!;

    public static ConfigEntry<bool> EnablePerFrameHarmonyThrottles { get; private set; } = null!;
    ...
    public static ConfigEntry<bool> EnableParticleAutoLifecycleThrottle { get; private set; } = null!;

    public static ConfigEntry<int> ParticleAutoLifecycleInterval { get; private set; } = null!;

    public static ConfigEntry<bool> EnableParticleCollisionDelayFix { get; private set; } = null!;
    ...
    public static ConfigEntry<bool> EnableColorCurvesManagerIdleSkip { get; private set; } = null!;
    ...
    {
        EnableHighFpsCompatibilityMode = config.Bind(
            "General",
            "EnableHighFpsCompatibilityMode",
            true,
            "Recommended for 120+ FPS. Skips injecting heavy per-frame Harmony patches (postprocess, idle skips, particle throttles). Keeps low-cost combat/water/tk2d fixes only. Restart game after changing.");

        EnablePerFrameHarmonyThrottles = config.Bind(
            "General",
            "EnablePerFrameHarmonyThrottles",
            false,
            "Master switch for distance/frame throttles. Only applies when High FPS Compatibility Mode is OFF.");
        ...
        EnableParticleAutoLifecycleThrottle = config.Bind(
            "Particles",
            "EnableParticleAutoLifecycleThrottle",
            false,
            "Runs ParticleSystem auto recycle/destroy/deactivate polling less often. This only delays cleanup by a few frames.");

        ParticleAutoLifecycleInterval = config.Bind(
            "Particles",
            "ParticleAutoLifecycleInterval",
            4,
            new ConfigDescription(
                "Frame interval used for ParticleSystemAutoRecycle, ParticleSystemAutoDestroy, and ParticleSystemAutoDeactivate.",
                new AcceptableValueRange<int>(2, 12)));

        EnableParticleCollisionDelayFix = config.Bind(
            "Particles",
            "EnableParticleCollisionDelayFix",
            true,
            "Fixes DisableParticleCollisonDelay so particle collision disables after the accumulated delay instead of comparing only Time.deltaTime.");
        ...
        EnableColorCurvesManagerIdleSkip = config.Bind(
            "PostFX",
            "EnableColorCurvesManagerIdleSkip",
            false,
            "Skips ColorCurvesManager.Update while factor/saturation values are unchanged, avoiding per-frame curve pairing allocations.");
    ...
    public static void SubscribeToChanges()
    {
        ...
        EnableAggressiveOffscreenCulling.SettingChanged += OnOffscreenCullingSettingChanged;
    }

    private static void OnOffscreenCullingSettingChanged(object? sender, EventArgs e)
    {
